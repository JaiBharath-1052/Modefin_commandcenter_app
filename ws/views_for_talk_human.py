...
class wsuser(WebSocketEndpoint):
    ...
    async def on_receive(self, websocket, text_data):
		...
		# if (action in ["input.unknown", "nlu_fallback", "out_of_scope"]) or (action is None): #commented by modefin
                if (action in ["input.unknown", "nlu_fallback", "out_of_scope"]) or (action is None): #commented by modefin
                    ...
                    # Server to visitor
                    metadata = {"contentType": "300", "payload": [{"title": "Talk to Human Agent"}], "templateId": "6"}
                    #button = {"title": "Talk to Human Agent"} # added by modefin
                    fe_visitor_response = {
                                           'type' : 'botMessage',
                                                'data': {
                                                    'message': "Sorry, I wasn't able to understand your query. Could you rephrase it? \n\n<br>       OR",
                    ...
                    await self.group_send(ws_list_visitor, fe_visitor_response)
                    log_message(10,f" fe_visitor_response  ={fe_visitor_response}")
                    ...
                    if self.availableAgent['agentAvailable']:
                        ...
                        #if self.availableAgent or agent_ws_list:
                            #await self.group_send(ws_list_visitor, fe_visitor_response)				
