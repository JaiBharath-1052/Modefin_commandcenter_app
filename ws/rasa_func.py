...
async def rasa_response(message="Hi", sender = None):
    ...
    log_message(10,f" rasa respnse  ={response}")
    ...
    if len(response) > 0:
        ...
        if custom is not None:
            ...
            log_message(10,f" intentName &&  intentDetectionConfidence  ={intentName,intentDetectionConfidence}")
        else:
            intentName = "Other"
            intentDetectionConfidence = 100

        text = get_key(response, "text")
        log_message(10,f" text in rasa_func : {text}")
        ...
        log_message(10,f" condition: if response is [] : {text}")
        intentName = "out_of_scope"
        intentDetectionConfidence = 30
