...
class wsuser(WebSocketEndpoint):
    ...
    async def on_connect(self, websocket):
        ...
        if self.talkto == 'agent':
            ...
            agent_name=self.availableAgent['agentDetails']['name']
            log_message(10,f" agent_name details ={agent_name}")
    ...
    async def on_receive(self, websocket, text_data):
        ...
        log_message(10,f" User message ={text_data_json}")        
        ...
        if msgType == "visitorActivity":
                ...
                # From second unhandled message from the user
                    ...
                    fe_server_response = {
                                    ...
                                    'data' : {
                                                ...
                                                'message' : f'You are connected to an agent {self.agentName}',
            ...
            if self.talkto == "bot":
                ...
                if text in ["custom_button","custom_button2","custom_link","custom_Sbutton", "custom_form"]:
                    log_message(10,f" output of Remove from production ={action}")
                ...
                if (action in ["input.unknown", "nlu_fallback", "out_of_scope"]) or (action is None):
                    log_message(10,f" response from action ={action}")
                    # Send the client a notification that we are asking for an agent
                    # Set variables to for upcoming messages
                    self.talkto = "agent"  # This variable will make sure all messages after first unknow messages are sent to agent

                    # Server to visitor  
                    text = text_data_json.get('data').get('message', None) # user message
                    log_message(10,f" text in views.py111111111 ={text}")

                    #metadata = {"contentType": "300", "payload": [{"title": "Do you want to talk with Human Agent?", "message": text}], "templateId": "6"}
                    fe_visitor_response = {
                                           'type' : 'botMessage',
                                                'data': {
                                                    #'message': "Could not understand your query, transferring the chat to agent",
                                                    'message': "Transferring the chat to an agent",
                                                    #'metadata': metadata, # added by modefin
                                                    'messageTime': servertime()
                                                }
                                            }
                    await self.group_send(ws_list_visitor, fe_visitor_response)
                    log_message(10,f" fe_visitor_response && ws_list_visitor   ={fe_visitor_response,ws_list_visitor}")
                    
                    ## add condition here for agent button as "Talk to Human Agent"
                    #agent_res=(metadata.get("payload"))  # payload
                    #agent_text=agent_res[0].get("message")

                    #text = text_data_json.get('data').get('message', None) # user message
                    #log_message(10,f" text and agent_text in views ={agent_text,text}")

                    #if text == agent_text:                    	
                    	#log_message(10,f" text in views.py2222222 ={text}")

                    

                    self.availableAgent = agentactivities_redis.get_agent()
                    log_message(10,f" self availableAgent in views.py ={self.availableAgent}")
                    ...
                    if self.availableAgent['agentAvailable']:
                        ...
                        if agent_ws_list:
                        ...
                        else: ## when agents are not available
                            fe_visitor_response = {
                                            'type' : 'serverActivity',
                                            'subType' : 'agentAssigned',
                                            'data' : {
                            ...
                            await self.group_send(ws_list_visitor, fe_visitor_response)
                    else: # When agent busy/ not available
            ...
            # This section is in multiple changes( Check the other section too )
                ...
                else:  ## when agents are not availabele
                    fe_visitor_response = {
                                       'type' : 'serverActivity',
                                       'subType' : 'agentAssigned',
                                       'data' : {
...
class wsagent(WebSocketEndpoint):
    ...
    log_message(10,f" wsagents::::: ={jwtalgo}")
    
    async def group_send(self, websockets, message):
        log_message(10,f" websockets and message ::::: ={websockets, message}")
        if isinstance(websockets, list):
            for ws in websockets:
                await ws.send_text(json.dumps(message))
        else:
    ...
    async def validate_jwt(self, websocket):
        try:
            ...
            log_message(10,f" self.agentID in views.pyyyyyyy::::: ={self.agentID}") # print(f"AgentID: {self.agentID}")
