...
def RedisClient():
	...
	#log_message(10,f"host in additionalfunc.py={host,port,password}")
	try:
		RC = Client(host=host, port=port, decode_responses=True, password=password)
		log_message(10,f" results of RC ={RC}")
		return RC
...
class agentactivities:
	...
	def addjson(self, jsonname, jsonobj, path = Path.rootPath()):
		try:
			...
			log_message(10,f" results of res in addjson fun ={res}")
			return res
		except Exception as e:
			return str(e)
	...
	def getjson(self, jsonname, path='.'):
		try:
			...
			log_message(10,f"response of getjson jsonobjjjjj ={jsonobj}")
			return jsonobj
		except Exception as e:
			return str(e)
	...
	def visitorTalkto(self, visitorID):
		...
		visitor = None # visitorID ##  
		# visitor = self.getjson(RedisJSONName).get(visitorID, None)
		log_message(10,f"visitor value ={visitor}")
		newlasttime = int(time.time())
		welcomeMessage = True
		if visitor:
                   	#log_message(10,f"inside visitor val")
			lasttime = visitor.get('lasttime', None)	
			if (lasttime + 0) > newlasttime: # 1800 (seconds i.e 30 mins) can come from DB 
				#log_message(10,f"inside visitor if condition")
				talkto = visitor.get('talkto', None)
				welcomeMessage = False
			else:
				#log_message(10,f"inside visitor else condition")
				talkto = "bot"
