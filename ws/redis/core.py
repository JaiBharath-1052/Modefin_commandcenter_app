...
async def messages_json(messages):
    msg_dict = {}
    for msg_id, msg in messages:
        msg_id = msg_id.decode('utf8')
        msg = [x.decode('utf8') for x in msg]
        msg_dict[msg_id] = {}
        ll = [msg_dict[msg_id].__setitem__(msg[x], msg[x + 1]) for x in range(0, len(msg), 2)]
    return msg_dict
...
class ConnectionPool:
    ...
    async def pop(self, loop=None):
        ...
        if not conns:
            conns.append(await aioredis.create_redis_pool(**self.host, loop=loop))
        conn = conns.pop()
        if conn.closed:
...
class RedisChannelLayer(BaseChannelLayer):
    ...
    ### Stream support Extension ###
    async def save_stream(self, channel, fields):
        """
        Add message to the persistent stream
        """
        ...
        async with self.connection(self.pool_index) as conn:
            return conn.execute(b'XADD', channel, "*", *args)

        # message = await message_json(message)
        # return message
